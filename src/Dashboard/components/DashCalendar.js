...
export default class DashboardCalendar extends React.Component {
	...
	renderData() {
		...
		return <div>{allDataSelected}</div>;
	}
	...
	render() {
		...
		for (let i = 0; i < this.firstDayOfMonth(); i++) {
			blanks.push(
				<li key={i * 80} className="emptySlot">
					{''}
				</li>,
			);
		}
			...
			return (
				<section className="main-content">
					<div className="dashcalendar">
						<DashPopup
							keys={trElems}
							popup={this.popupClose.bind(this)}
							allDay={this.renderData()}
						/>
					</div>
				</section>
			);
		...
		return (
			<section className="main-content">
				<div className="dashcalendar">
					...
					<ul className="dash-weekday">{weekdays}</ul>
					<ul onClick={this.popupOpen} className="days">
						{trElems}
					</ul>
				</div>
			</section>
